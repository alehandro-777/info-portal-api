...
exports.table = async (req, res) => { 
    ...
    try {
        ...
        const data = await dataService.selectValueGroupByObjectParameterAsync(objects, parameters, new Date(from), new Date(to));
        return res.status(200).json({ data });
    } 
    catch (error) {
        return res.status(500).json({ message: error});
...
exports.stats = async (req, res) => { 
    ...
    try {
        ...
        const data = await dataService.statistics(objects, parameters, new Date(from), new Date(to));
        return res.status(200).json({ data });
    } 
    catch (error) {
        return res.status(500).json({ message: error});

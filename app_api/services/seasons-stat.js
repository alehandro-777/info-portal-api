...
async function create (seasonId) {
    ...
    result.wi_ym = tempDataYM.filter(d=> d._id.object == 906021 && d._id.parameter == 902);
    result.wi = tempData.filter(d=> d._id.object == 906021 && d._id.parameter == 902);

    result.in_ym = tempDataYM.filter(d=> d._id.object == 906021 && d._id.parameter == 901);
    result.in = tempData.filter(d=> d._id.object == 906021 && d._id.parameter == 901);
    ...
    //bad CH calc
    let tempDataBadCH_ym = await smartFiltersService.smartFilterGroupByObjParYearMonth([906021], [901, 902], season.start, season.end, 999082, 13, {$gt: 0});
    let tempDataBadCH = await smartFiltersService.smartFilterGroupByObjPar([906021], [901, 902], season.start, season.end, 999082, 13, {$gt: 0});
    
    //bad H2O calc
    let tempDataBadH2O_ym = await smartFiltersService.smartFilterGroupByObjParYearMonth([906021], [901, 902], season.start, season.end, 999082, 29, {$gt: -8});
    let tempDataBadH2O = await smartFiltersService.smartFilterGroupByObjPar([906021], [901, 902], season.start, season.end, 999082, 29, {$gt: -8});
    ...
    let tempDataSamoplyv = await smartFiltersService.smartFilterGroupByObjParNotIn([906021, 6090520, 6090411], [901, 902, 63], season.start, season.end, 906014, 30, {$gt: 0});

    //pal pas >0 inverse "Compress"
    let tempDataCompress_ym = await smartFiltersService.smartFilterGroupByObjParYearMonth([906021, 6090520, 6090411], [901, 902, 63], season.start, season.end, 906014, 30, {$gt: 0});
    let tempDataCompress = await smartFiltersService.smartFilterGroupByObjPar([906021, 6090520, 6090411], [901, 902, 63], season.start, season.end, 906014, 30, {$gt: 0});

    result.wi_bad_ch_ym = tempDataBadCH_ym.filter(d=> d._id.object == 906021 && d._id.parameter == 902);
    result.wi_bad_ch = tempDataBadCH.filter(d=> d._id.object == 906021 && d._id.parameter == 902);

    result.in_bad_ch_ym = tempDataBadCH_ym.filter(d=> d._id.object == 906021 && d._id.parameter == 901);
    result.in_bad_ch = tempDataBadCH.filter(d=> d._id.object == 906021 && d._id.parameter == 901);

    result.wi_bad_h2o_ym = tempDataBadH2O_ym.filter(d=> d._id.object == 906021 && d._id.parameter == 902);
    result.wi_bad_h2o = tempDataBadH2O.filter(d=> d._id.object == 906021 && d._id.parameter == 902);

    result.in_bad_h2o_ym = tempDataBadH2O_ym.filter(d=> d._id.object == 906021 && d._id.parameter == 901);
    result.in_bad_h2o = tempDataBadH2O.filter(d=> d._id.object == 906021 && d._id.parameter == 901);


    result.wi_samoplyv_ym = tempDataSamoplyv_ym.filter(d=> d._id.object == 906021 && d._id.parameter == 902);
    result.wi_samoplyv = tempDataSamoplyv.filter(d=> d._id.object == 906021 && d._id.parameter == 902);
    ...
    result.in_samoplyv_ym = tempDataSamoplyv_ym.filter(d=> d._id.object == 906021 && d._id.parameter == 901);
    result.in_samoplyv = tempDataSamoplyv.filter(d=> d._id.object == 906021 && d._id.parameter == 901);
...
function findByObjectParameter(array, object, parameter) {
    return array.find(m=> m._id.object == object && m._id.parameter == parameter);
}
